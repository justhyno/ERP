...
import com.xhuma.erp.config.KafkaProperties;
...
class XhumaErpKafkaResourceIT {
    ...
    void setup() {
        ...
        XhumaErpKafkaResource kafkaResource = new XhumaErpKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/xhuma-erp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/xhuma-erp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
